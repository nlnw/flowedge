...
server.resource("anvil_status", "anvil://status", async (uri: any) => {
...
server.tool(
  ...
  async ({ dependency, version }: { dependency: string; version?: string }) => {
    const installed = await checkFoundryInstalled();
    if (!installed) {
      return {
        content: [{ type: "text", text: FOUNDRY_NOT_INSTALLED_ERROR }],
      ...
      try {
        ...
        if (contractData && contractData.functionSignatures) {
          ...
          const funcData = contractData.functionSignatures.find(
            (f: any) => f.name.toLowerCase() === functionName.toLowerCase(),
          );
