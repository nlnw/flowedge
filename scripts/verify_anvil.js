  ...
  try {
    ...
    while (retries > 0) {
      try {
        ...
        console.log(
          `Waiting for Anvil to be ready... (${retries} retries left)`,
        );
    ...
    if (!chainInfo) {
      ...
      throw new Error(
        `Unknown chain ID ${chainId}. Expected one of: ${validChainIds}`,
      );
    }
      ...
      try {
        if (addresses.hasContract(contractName)) {
          ...
          try {
            ...
            console.log(
              `⚠️  Contract ${contractName} exists but couldn't read ERC20 data (might not be an ERC20 token)`,
            );
            testedCount++;
          }
        } else {
...
async function main() {
  ...
  if (result && result.success) {
    ...
    console.log(
      "Press Ctrl+C in the Anvil terminal to stop the server when done.",
    );
    console.log(
