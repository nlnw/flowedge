...
if (!existsSync(ABIS_DIR)) {
  console.error(
    'Error: ABIs directory does not exist. Run "bun run build:abi" first.',
  );
  process.exit(1);
}
...
if (!existsSync(ADDRESSES_DIR)) {
  console.error(
    'Error: Addresses directory does not exist. Run "bun run build:addressutils" first.',
  );
  process.exit(1);
}
...
function loadAddresses() {
  try {
    ...
    if (!existsSync(addressMappingFile)) {
      console.warn(
        "Warning: Address mapping file not found. Addresses will not be included.",
      );
      return {};
    }
    ...
    const contractAddressMatch = mappingContent.match(
      /export const CONTRACT_ADDRESSES = ({[\s\S]*?}) as const;/,
    );
    if (!contractAddressMatch) {
      console.warn(
        "Warning: Could not parse CONTRACT_ADDRESSES from mapping.ts",
      );
      return {};
    }
    ...
    try {
      ...
      console.log(
        `Loaded addresses for ${Object.keys(addressData).length} contracts`,
      );
      return addressData;
    } catch (parseError) {
      console.warn(
        "Warning: Could not parse address data:",
        parseError.message,
      );
      return {};
    }
...
function loadOriginAddresses() {
  try {
    ...
    const originAddressMatch = mappingContent.match(
      /export const ORIGIN_CONTRACT_ADDRESSES = ({[\s\S]*?}) as const;/,
    );
    ...
    try {
      ...
      console.log(
        `Loaded origin addresses for ${Object.keys(originData).length} contracts`,
      );
      return originData;
    } catch (parseError) {
      console.warn(
        "Warning: Could not parse origin address data:",
        parseError.message,
      );
      return {};
    }
...
function extractFunctionSignatures(abi) {
  ...
  for (const item of abi) {
    if (item.type === "function") {
      ...
      const outputTypes = (item.outputs || [])
...
function extractContractMetadata(content) {
  ...
  const titleMatches =
    commentBlock.match(/@title\s+(.*?)(?=\s*\*\s*@|\s*\*\/)/gs) || [];
  const noticeMatches =
    commentBlock.match(/@notice\s+(.*?)(?=\s*\*\s*@|\s*\*\/)/gs) || [];
  const devMatches =
    commentBlock.match(/@dev\s+(.*?)(?=\s*\*\s*@|\s*\*\/)/gs) || [];
  ...
  for (const network of NETWORKS) {
    ...
    const networkRegex = new RegExp(
      `@custom:${network}\\s+(0x[a-fA-F0-9]{40})`,
      "gi",
    );
    ...
    const originRegex = new RegExp(
      `@custom:${network}\\s+(ethereum|sepolia):(0x[a-fA-F0-9]{40})`,
      "gi",
    );
  ...
  const tags =
    tagsMatches.length > 0
      ? tagsMatches[0]
          .replace(/@custom:tags\s+/g, "")
          .trim()
          .split(",")
          .map((tag) => tag.trim())
      : [];
...
function generateContractDirectory() {
  ...
  for (const [key, contractInfo] of Object.entries(contractFiles)) {
    ...
    const abiKey = join(contractInfo.relativePath, `${fileName}.json`).replace(
      /\\/g,
      "/",
    );
    ...
    const contractName = interfaceMatch
      ? interfaceMatch[1]
      : contractMatch
        ? contractMatch[1]
        : fileName;
    ...
    if (addresses[contractName] || addresses[addressMatchName]) {
      contractAddresses =
        addresses[contractName] || addresses[addressMatchName];
      console.log(
        `Found addresses for ${contractName}: ${JSON.stringify(contractAddresses)}`,
      );
    }

    // Look up origin addresses
    if (originAddresses[contractName] || originAddresses[addressMatchName]) {
      contractOriginAddresses =
        originAddresses[contractName] || originAddresses[addressMatchName];
      console.log(
        `Found origin addresses for ${contractName}: ${JSON.stringify(contractOriginAddresses)}`,
      );
    }
    ...
    if (
      ...
      metadata &&
      Object.keys(metadata.networkAddresses).length > 0
    ) {
      contractAddresses = metadata.networkAddresses;
      console.log(
        `Using doccomment addresses for ${contractName}: ${JSON.stringify(contractAddresses)}`,
      );
    }

    if (
      Object.keys(contractOriginAddresses).length === 0 &&
      metadata &&
      Object.keys(metadata.originAddresses).length > 0
    ) {
      contractOriginAddresses = metadata.originAddresses;
      console.log(
        `Using doccomment origin addresses for ${contractName}: ${JSON.stringify(contractOriginAddresses)}`,
      );
    }
...
function generateSchema() {
  return {
    ...
    contractStructure: {
      ...
      originAddresses:
        "object - Origin chain addresses for cross-chain operations",
...
function generateDiverseSample(contractDir) {
  ...
  for (const targetContext of priorityContexts) {
    const contractInContext = contractDir.find(
      (c) =>
  ...
  while (diverseContracts.length < DIVERSE_SIZE) {
    const remaining = contractDir.find(
      (c) => !diverseContracts.includes(c) && c.hasAbi && c.functionCount > 2,
    );
...
const sampleContracts = contractDir
  .filter((c) => c.hasAbi)
  .slice(0, SAMPLE_SIZE);
...
console.log(
  `  - Contracts with ABIs: ${contractDir.filter((c) => c.hasAbi).length}`,
);
console.log(
  `  - Contracts with addresses: ${contractDir.filter((c) => c.hasAddresses).length}`,
);
console.log(
  `  - Contracts with origin addresses: ${contractDir.filter((c) => c.hasOriginAddresses).length}`,
);
console.log(
  `  - Total function signatures: ${contractDir.reduce((sum, c) => sum + c.functionCount, 0)}`,
);
...
NETWORKS.forEach((network) => {
  ...
  const withOrigin = contractDir.filter(
    (c) => c.originAddresses[network],
  ).length;
  console.log(
    `  - ${network}: ${withAddress} regular, ${withOrigin} origin addresses`,
  );
});
