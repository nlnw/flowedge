...
function getAllSolidityFiles(dir, result = []) {
  ...
  for (const file of files) {
    ...
    if (stats.isDirectory() && file !== "utils") {
      // Skip the utils directory to avoid circular references
...
function extractContractName(filePath) {
  try {
    ...
    for (const line of lines) {
      ...
      const modifiedInterfaceMatch = trimmedLine.match(
        /^(?:abstract\s+)?interface\s+(\w+)/,
      );
      if (modifiedInterfaceMatch && modifiedInterfaceMatch[1]) {
        return modifiedInterfaceMatch[1];
      }

      const modifiedContractMatch = trimmedLine.match(
...
library ${networkName}OriginAddresses {\n`;
  ...
  // Sort contracts by name for consistent output
  const sortedContracts = Object.entries(originContracts).sort(([a], [b]) =>
    a.localeCompare(b),
  );
...
${Object.entries(addressMapping)
  ...
  .map(([name, addrs]) => {
    ...
    const tatara = addrs.tatara
      ? `"tatara": "${addrs.tatara}"`
      : '"tatara": null';
    const katana = addrs.katana
      ? `"katana": "${addrs.katana}"`
      : '"katana": null';
    const bokuto = addrs.bokuto
      ? `"bokuto": "${addrs.bokuto}"`
      : '"bokuto": null';
...
function main() {
  ...
  for (const filePath of solidityFiles) {
    ...
    for (const [network, address] of Object.entries(addresses)) {
      if (networkContracts[network]) {
        ...
        if (
          networkContracts[network][contractName] &&
          networkContracts[network][contractName] !== address
        ) {
          console.log(
            `  Warning: Duplicate contract name "${contractName}" with different addresses:`,
          );
          console.log(
            `    Existing: ${networkContracts[network][contractName]}`,
          );
          console.log(
            `    New: ${address} (from ${relative(CONTRACTS_DIR, filePath)})`,
          );
        ...
        totalAddressesFound++;
        console.log(
          `  Found ${network}: ${contractName} -> ${address} (${relative(CONTRACTS_DIR, filePath)})`,
        );
      }
    }
    ...
    for (const [network, addressData] of Object.entries(originAddresses)) {
      if (originNetworkContracts[network]) {
        // Check for duplicates
        if (
          originNetworkContracts[network][contractName] &&
          originNetworkContracts[network][contractName].address !==
            addressData.address
        ) {
          console.log(
            `  Warning: Duplicate origin contract name "${contractName}" with different addresses:`,
          );
          console.log(
            `    Existing: ${originNetworkContracts[network][contractName].address} (${originNetworkContracts[network][contractName].originChain})`,
          );
          console.log(
            `    New: ${addressData.address} (${addressData.originChain}) (from ${relative(CONTRACTS_DIR, filePath)})`,
          );
        ...
        console.log(
          `  Found ${network} origin (${addressData.originChain}): ${contractName} -> ${addressData.address} (${relative(CONTRACTS_DIR, filePath)})`,
        );
      }
    }
  }
  ...
  for (const network of NETWORKS) {
    ...
    const content = generateOriginAddressLibrary(
      network,
      originNetworkContracts[network],
    );
    writeFileSync(filePath, content);

    const contractCount = Object.keys(originNetworkContracts[network]).length;
    console.log(
      `  Generated ${fileName} with ${contractCount} origin contracts`,
    );
  }
  ...
  const tsContent = generateTypeScriptMapping(
    networkContracts,
    originNetworkContracts,
  );
